...
if (place_meeting(x, y, obj_boss_hurt) && invencibility <= 0)
{
	
	if (rubies != 0)
	{
		audio_play_sound(snd_big_sword_hit, 1, false)
...
if (obj_boss.state == BossStates.special)
{
	
	if (place_meeting(x, y, obj_boss) && invencibility <= 0)
	{
...
switch(state)
{
	...
	case(global.ATTACKSLASH): // Attacking
		...
		//activates attack
		ProcessAttackBoss(spr_warrior_attack, spr_warrior_attackHB);
	...
	case(global.ATTACKCOMBO): //Attack combo
		...
		//activates attack
		ProcessAttackBoss(spr_warrior_attackCombo, spr_warrior_attackComboHB);
	...
	case(playerStates.hurt):
		...
		if (gotHit == false) //Code to be executed right after getting hit
		{
			...
			image_index = 0;
			_dir = sign(x - instance_nearest(x, y, obj_boss).x); // Detects where the enemy is in relation of the player
			vspd = -6;
			gotHit = true;
		...
		else if (grounded)
		{
			...
			invencibility = 80; // Gives the player a second of invencibility
